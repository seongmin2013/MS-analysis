...
if( 'PrecursorError(Da)' in header_list ):
    pmerror_idx = header_list.index('PrecursorError(Da)')
...
for line in f_in:
    ...
    filename = 'data/'+ os.path.basename(tokens[filename_idx]).split('.')[0]
    scan_id = int(tokens[scan_id_idx].split('scan=')[1])
