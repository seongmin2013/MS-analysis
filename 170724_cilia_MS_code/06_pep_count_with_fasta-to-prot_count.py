...
for prot_id in prot_list.keys():
    if( prot_id.startswith('rv_') or prot_id.startswith('xf_') or prot_id.startswith('rev_') or prot_id.startswith('DECOY_')):
        continue
...
filename_prot_count = filename_pep_count.replace('#pepSpectrum.txt','#prot')
